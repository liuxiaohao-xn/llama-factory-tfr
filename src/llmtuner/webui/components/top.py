...
def create_top() -> Dict[str, "Component"]:
    ...
    model_paths = [
        "/asr_lu/user/liuxiaohao/llms/THUDM/chatglm3-6b"
    ]
    with gr.Row():
        lang = gr.Dropdown(choices=["en", "zh"], scale=1, visible=False)
        model_name = gr.Dropdown(choices=available_models, scale=3)
        # model_path = gr.Textbox(scale=3)
        model_path = gr.Dropdown(choices=model_paths, scale=3, visible=False)

    with gr.Row():
        finetuning_type = gr.Dropdown(choices=METHODS, value="lora", scale=1, visible=False)
    ...
    with gr.Accordion(label="Model config (LLaMA only)", open=False, visible=False) as llama_tab:
        with gr.Row():
            with gr.Column():
