...
def create_train_tab(engine: "Engine") -> Dict[str, "Component"]:
    ...
    gr.File(label="训练样本格式下载", interactive=False, value=r"yh_data/sample.jsonlines")
    with gr.Row():
        upload_button = gr.UploadButton(
            label="上传训练数据，格式为.jsonlines文件",
            file_types=[".jsonlines"],
            scale=5
        )
        file_output = gr.TextArea(label="保存为数据集名称")
        
    from llmtuner.webui.self_define import upload_file
    upload_button.upload(upload_file, upload_button, file_output)
    with gr.Row():
        training_stage = gr.Dropdown(
            choices=list(TRAINING_STAGES.keys()), value=list(TRAINING_STAGES.keys())[0], scale=2, visible=False
        )
        dataset_dir = gr.Textbox(value=DEFAULT_DATA_DIR, scale=2, visible=False)
        # dataset = gr.Dropdown(multiselect=True, scale=5)
        dataset = gr.Dropdown(multiselect=True, scale=5, max_choices=1)
        preview_elems = create_preview_box(dataset_dir, dataset)
        refresh_btn = gr.Button(value="刷新数据集", scale=1)
    ...
    refresh_btn.click(list_dataset, [dataset_dir, training_stage], [dataset], queue=False)

    with gr.Row():
        cutoff_len = gr.Slider(value=1024, minimum=4, maximum=8192, step=1, visible=False)
        learning_rate = gr.Textbox(value="5e-5", visible=False)
        num_train_epochs = gr.Textbox(value="30")
        max_samples = gr.Textbox(value="100000", visible=False)
        compute_type = gr.Radio(choices=["fp16", "bf16"], value="fp16", visible=False)
    ...
    with gr.Accordion(label="Advanced config", open=False, visible=False) as advanced_tab:
        with gr.Row():
    ...
    with gr.Accordion(label="LoRA config", open=False, visible=False) as lora_tab:
        with gr.Row():
    ...
    with gr.Accordion(label="RLHF config", open=False, visible=False) as rlhf_tab:
        with gr.Row():
